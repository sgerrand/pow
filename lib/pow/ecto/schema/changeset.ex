defmodule Pow.Ecto.Schema.Changeset do
  ...
  defp maybe_validate_current_password(
         %Changeset{data: %{password_hash: nil}} = changeset,
         _config
       ),
       do: changeset
  ...
  defp validate_current_password(
         %Changeset{data: user, changes: %{current_password: password}} = changeset,
         config
       ) do
    user
  ...
  defp get_password_hash_functions(config) do
    case Config.get(config, :password_hash_verify) do
      nil ->
        fallback_get_password_hash(config) || {&Password.pbkdf2_hash/1, &Password.pbkdf2_verify/2}
  ...
  defp fallback_get_password_hash(config) do
    case Config.get(config, :password_hash_methods) do
      ...
      functions ->
        IO.warn(
          "use of `:password_hash_methods` config value is deprecated, use `:password_hash_verify` instead"
        )
